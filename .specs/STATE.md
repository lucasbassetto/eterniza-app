...
- **Feature ativa:** `etapa-5-fluxo-convidado` — implementada e verificada (Verifier PASS de primeira, 91 testes, sensor 6/6); commits cd92639 (GUEST-02), 9e498f7 (GUEST-01), 7723853 (GUEST-03). **Pendente: UAT interativo** (fluxo real no iPhone: convite → nome → câmera; slug inexistente → erro elegante). Requer um evento de teste criado no backend (`POST /api/events` com o hostToken).
