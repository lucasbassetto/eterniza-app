...
- **Feature ativa:** nenhuma — próxima é a Etapa 6 (câmera VisionCamera). ⚠️ **Etapa 6 exige dev build** (`expo-dev-client`): VisionCamera não roda no Expo Go. Build iOS a partir do Windows = EAS Build (nuvem); requer decisão do usuário sobre conta Apple Developer. AD-006 (SDK 54) deve ser reavaliada aqui.
